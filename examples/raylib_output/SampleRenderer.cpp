...
SampleRenderer::SampleRenderer() {
    ...
    logging::info("creating optix context");
    createContext();

    logging::info("creating module");
    createModule();

    logging::info("creating raygen programs");
    createRaygenPrograms();

    logging::info("creating miss programs");
    createMissPrograms();

    logging::info("creating hitgroup programs");
    createHitgoupPrograms();

    logging::info("create pipeline");
    createPipeline();

    logging::info("build SBT");
    buildSBT();

    logging::info("alloc launch params buffer");
    launchParamsBuffer.alloc<LaunchParams>(1);
}

void SampleRenderer::initOptix() {

    logging::info("initialising optix");
    ...
    logging::info("found {} CUDA devices", num_devices);


    OP_CHECK_FATAL(optixInit());
    logging::info("successfully initialised optix");
}
...
static void context_log_callback(unsigned int level,
    ...
    switch(level) {
        case 0:
            logging::info("{} {:12} {}", level, tag, message);
            break;
        default:
            logging::info("{} {:12} {}", level, tag, message);
            break;
...
void SampleRenderer::createContext() {
    ...
    logging::info("device {}", deviceProps.name);
...
void SampleRenderer::createModule() {
    ...
    logging::info("cuda incl: {}", CUDA_INCLUDE_DIRS);
    ...
    if (logsize > 0) logging::info("{}", logBuffer);
}
...
void SampleRenderer::createRaygenPrograms() {
    ...
    if (logsize > 1) logging::info("{}", logBuffer);
...
void SampleRenderer::buildSBT() {
    ...
    logging::info("alloc and upload raygen records");
    ...
    logging::info("alloc and upload miss records");
    ...
    for(int i = 0; i < numObjects; ++i) {
        ...
        logging::info("object type: {}", objectType);
        logging::info("hitgroup_size: {}", hitgroupPGs.size());
        OP_CHECK_FATAL(optixSbtRecordPackHeader( hitgroupPGs[objectType],&record));
        record.objectID = i;
        hitgroupRecords.push_back(record);
    }
    logging::info("alloc and upload hit group records");
